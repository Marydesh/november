...
const shapeTests = [
    ['Triangle', Triangle, (color, textColor) => {
        return `<svg version="1.1"
        width="300" height="200"
        xmlns="http://www.w3.org/2000/svg">
        <polygon points="100 30, 200 200, 0 200" fill="${color}"/>
        <text x="100" y="180" font-size="60" text-anchor="middle" fill="${textColor}">SVG</text>
        </svg>`;
    }],
    ['Circle', Circle, (color, textColor) => {
        return `<svg version="1.1"
        width="300" height="200"
        xmlns="http://www.w3.org/2000/svg">
...
shapeTests.forEach(([name, Shape, svg]) => {
    let shape = new Shape('black', 'SVG', 'white');
